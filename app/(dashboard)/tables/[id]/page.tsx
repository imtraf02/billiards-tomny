...
export default function TableDetailPage() {
		...
		if (table.status === "occupied") {
			const interval = setInterval(() => {
				...
				const diff = Math.floor(
					(now.getTime() - table.startTime.getTime()) / 1000,
				);
	...
	const addToCart = (
		product: (typeof drinkProducts)[0] | (typeof foodProducts)[0],
	) => {
		setCart((prev) => {
	...
	const handleReserve = () => {
		...
		setReservationTime("");
		alert(
			`Đã đặt bàn ${table.name} cho ${customerName} vào lúc ${reservationTime}`,
		);
	};
	...
	const handleEnd = () => {
		...
		if (
			confirm(
				`Kết thúc sử dụng bàn ${table.name}. Tổng thanh toán: ${total.toLocaleString("vi-VN")}₫`,
			)
		) {
	...
	const handleCheckout = () => {
		...
		alert(
			`Xác nhận đơn hàng: ${cart.length} món - Tổng: ${total.toLocaleString("vi-VN")}₫`,
		);
		setCart([]);
	};
					...
					<div className="flex items-center justify-between">
						...
						<div className="text-right">
							<div className="text-2xl font-bold">
								{table.pricePerHour.toLocaleString("vi-VN")}₫/giờ
							</div>
							<div
						...
						<CardContent>
							...
							{cart.length > 0 && (
								<div className="mt-6 pt-6 border-t border-border">
									...
									<div className="text-sm text-muted-foreground">
										Nhấn vào món để thêm vào giỏ, nhấn vào giỏ hàng để xem chi
										tiết
									</div>
								</div>
							)}
