...
export const createProductSchema = z.object({
	categoryId: z.string().min(1, { error: "Danh mục là bắt buộc" }),
	name: z
		.string()
		.min(1, { error: "Tên sản phẩm là bắt buộc" })
		.max(100, { error: "Tên sản phẩm quá dài" }),
	price: z.coerce.number().int().positive({ error: "Giá bán phải lớn hơn 0" }),
	cost: z.coerce
		.number()
		.int()
		.positive({ error: "Giá vốn phải lớn hơn 0" })
		.optional(),
	description: z.string().optional(),
	imageUrl: z.string().url({ error: "URL ảnh không hợp lệ" }).optional(),
