...
const generateMockFinancialData = (timeRange?: string) => {
    const baseRevenue = timeRange === 'YEAR' ? 150000000 : 
                      timeRange === 'MONTH' ? 12500000 : 
                      timeRange === 'WEEK' ? 3000000 : 1250000;
    ...
    return {
        ...
        expenseChart: [
            ...
            { name: "Tiện ích", value: baseRevenue * 0.10, color: "#FF8042" },
            { name: "Khác", value: baseRevenue * 0.15, color: "#8884D8" },
        ],
        topProducts: [
            { product: "Cà phê sữa", revenue: 4500000, cost: 1800000, profit: 2700000 },
        ...
        fixedExpenses: [
            { id: 1, name: "Tiền thuê mặt bằng", description: "Hàng tháng", amount: 5000000 },
            { id: 2, name: "Lương nhân viên", description: "Hàng tháng", amount: 15000000 },
...
const generateRevenueData = (timeRange?: string) => {
    const labels = timeRange === 'WEEK' ? ['T2', 'T3', 'T4', 'T5', 'T6', 'T7', 'CN'] :
                   timeRange === 'MONTH' ? ['Tuần 1', 'Tuần 2', 'Tuần 3', 'Tuần 4'] :
...
const generateMockDebts = (count: number = 10) => {
    const contacts = ['Nguyễn Văn A', 'Công ty ABC', 'Lê Thị B', 'Nhà cung cấp XYZ', 'Khách hàng VIP'];
...
export function useGetDebts(params?: TransactionParams) {
    return useQuery({
        ...
        queryFn: async () => {
            ...
            const summary = mockData.reduce((acc, debt) => {
