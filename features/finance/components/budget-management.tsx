...
export function BudgetManagement() {
	...
	const getStatus = (percentage: number) => {
		...
		if (percentage <= 90)
			return { label: "Sắp vượt", color: "text-amber-600", bg: "bg-amber-100" };
							...
							<CardContent className="pt-6">
								<div className="flex items-center justify-between">
									<div>
										...
										<p
											className={`text-2xl font-bold ${(budgets?.summary?.remaining || 0) >= 0 ? "text-green-600" : "text-red-600"}`}
										>
							...
							{budgets.recentTransactions.map((transaction: any) => (
								...
								>
									<div className="flex items-center gap-3">
										<div
											className={`p-2 rounded-full ${transaction.categoryColor} bg-opacity-20`}
										>
											<DollarSign
												className={`h-4 w-4 ${transaction.categoryColor}`}
											/>
										</div>
										<div>
