...
export function ProductCard({
	...
	const getStockStatus = () => {
		...
		if (product.currentStock <= product.minStock)
			return { status: "LOW", label: "Còn ít" };
		if (product.currentStock <= product.minStock * 2)
			return { status: "MEDIUM", label: "Sắp hết" };
				...
				<div className="space-y-2">
					...
					<div className="flex items-center justify-between">
						...
						<div className="flex items-center gap-1.5">
							<span
								className={`text-sm font-medium ${product.currentStock <= product.minStock ? "text-red-600" : ""}`}
							>
					...
					<div className="flex items-center justify-between">
						...
						<div className="flex items-center gap-1.5">
							<div
								className={`h-2 w-2 rounded-full ${stockColors[stockStatus.status]}`}
							/>
